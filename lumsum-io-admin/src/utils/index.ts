...
export function deleteS3File(key: string): Promise <string> {
    return new Promise((resolve, reject) => {
        if (!key) resolve("File removed");
...
export async function uploadS3File(key: string, body: object): Promise <any> {
...
export function generateKeyFromName(name): string {
    const ext: string = name.split(".").pop();
    return `${new Date().getTime()}.${ext}`;
}

export function getFallbackImg(): string {
